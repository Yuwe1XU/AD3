...
class GNNEncoder(nn.Module):
  ...
  def __init__(self, n_layers, hidden_dim, out_channels=1, aggregation="sum", norm="layer",
    ...
    if not node_feature_only:
      # self.pos_embed = PositionEmbeddingSine(hidden_dim // 2, normalize=False)
  ...
  def dense_forward(self, x, graph, timesteps, edge_index=None):
    ...
    # NEED A NEW g FOR TRANS E INTO ADJ
  ...
  def mbs_forward(self, x, graph, timesteps, edge_index=None):
    """
    Args:
        
        x: Input node coordinates (B x V x 2) B x V x 1??
        graph: Graph adjacency matrices (B x V x V) # graph None?
        timesteps: Input node timesteps (B)
        edge_index: Edge indices (2 x E) # Graph adjacency matrices (B x V x V)?
    Returns:
        Updated edge features (B x V x V)
    """
    # Embed edge features
    ...
    n = x.size(0) 
    m = x.size(1)
    x = x.reshape(batch_size, n // batch_size, m)
    e = self.edge_embed(self.edge_pos_embed(graph))
    time_emb = self.time_embed(timestep_embedding(timesteps, self.hidden_dim))
    
    graph = np.abs(graph.cpu())
  ...
  def forward(self, x, timesteps, graph=None, edge_index=None):
    if self.node_feature_only:
      ...
      if self.sparse:
        ...
        return self.mbs_forward(x, graph, timesteps, edge_index)
